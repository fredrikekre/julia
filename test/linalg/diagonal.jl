    ...
    @testset "Linear solve" begin
        for (v, U) in ((vv, UU), (view(vv, 1:n), view(UU, 1:n, 1:2)))
            ...
            if relty != BigFloat
                ...
                @test_throws SingularException ldiv!(Diagonal(fill(relty(0), n)), copy(v))
                b = rand(elty, n, n)
                b = sparse(b)
                @test ldiv!(D, copy(b)) ≈ Array(D)\Array(b)
                @test_throws SingularException ldiv!(Diagonal(fill(elty(0), n)), copy(b))
                b = view(rand(elty, n), collect(1:n))
                b2 = copy(b)
                c = ldiv!(D, b)
                d = Array(D)\b2
                @test c ≈ d
                @test_throws SingularException ldiv!(Diagonal(fill(elty(0), n)), b)
                b = rand(elty, n+1, n+1)
                b = sparse(b)
...
@testset "svdvals and eigvals (#11120/#11247)" begin
    ...
    Z2x3, Z3x2 = fill(0, (2,3)), fill(0, (3,2))
...
@testset "inverse" begin
    ...
    @test_throws SingularException inv(Diagonal(fill(0, n)))
...
for t in (Float32, Float64, Int, Complex{Float64}, Rational{Int})
    @test Diagonal(Matrix{t}[fill(t(1), 2, 2), fill(t(1), 3, 3)])[2,1] == fill(t(0), (3,2))
end
