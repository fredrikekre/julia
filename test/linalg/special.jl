...
@testset "Interconversion between special matrix types" begin
    ...
    @testset for isupper in (true, false)
        ...
        A = Bidiagonal(a, fill(0.0, n-1), ifelse(isupper, :U, :L)) #morally Diagonal
    ...
    A = SymTridiagonal(a, fill(0.0, n-1))
    @test Matrix(convert(Bidiagonal,A)) == Matrix(A)

    A = Tridiagonal(fill(0.0, n-1), [1.0:n;], fill(0.0, n-1)) #morally Diagonal
    ...
    A = Tridiagonal(fill(0.0, n-1), [1.0:n;], fill(1., n-1)) #not morally Diagonal
    @test Matrix(convert(Bidiagonal, A)) == Matrix(A)
    A = UpperTriangular(Tridiagonal(fill(0.0, n-1), [1.0:n;], fill(1., n-1)))
    @test Matrix(convert(Bidiagonal, A)) == Matrix(A)
    A = Tridiagonal(fill(1., n-1), [1.0:n;], fill(0.0, n-1)) #not morally Diagonal
    @test Matrix(convert(Bidiagonal, A)) == Matrix(A)
    A = LowerTriangular(Tridiagonal(fill(1., n-1), [1.0:n;], fill(0.0, n-1)))
    @test Matrix(convert(Bidiagonal, A)) == Matrix(A)
    @test_throws ArgumentError convert(SymTridiagonal,A)
