...
@testset for relty in (Int, Float32, Float64, BigFloat), elty in (relty, Complex{relty})
    ...
    @testset for uplo in (:U, :L)
        ...
        @testset "triu and tril" begin
            zerosdv = fill(elty(0), length(dv))
            zerosev = fill(elty(0), length(ev))
        ...
        @testset "diag" begin
            ...
            @test (@inferred diag(T,2))::typeof(dv) == fill(elty(0), n-2)
            ...
            @test (@inferred diag(G,2))::typeof(gdv) == GenericArray(fill(elty(0), n-2))
        end
