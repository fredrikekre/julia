...
@testset for relty in (Int, Float32, Float64, BigFloat), elty in (relty, Complex{relty})
    ...
    @testset for uplo in (:U, :L)
        ...
        @testset "Constructor and basic properties" begin
            ...
            @test size(T) == (n, n)
            @test Array(T) == diagm(dv => 0, ev => uplo == :U ? 1 : -1)
            @test Bidiagonal(Array(T), uplo) == T
            @test big.(T) == T
            @test Array(abs.(T)) == abs.(diagm(dv => 0, ev => uplo == :U ? 1 : -1))
            @test Array(real(T)) == real(diagm(dv => 0, ev => uplo == :U ? 1 : -1))
            @test Array(imag(T)) == imag(diagm(dv => 0, ev => uplo == :U ? 1 : -1))
        end
