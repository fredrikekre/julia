...
bool is_inbounds()
{
    ...
    bool inbounds = !this->inbounds.empty() && this->inbounds.back();
    if (this->inbounds.size() > 1)
        inbounds |= this->inbounds[this->inbounds.size()-2];
    return inbounds;
}

bool is_bounds_check_block()
{
    return !this->boundsCheck.empty() && this->boundsCheck.back();
}
